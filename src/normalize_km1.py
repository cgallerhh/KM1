...
from km1_common import GESCHLECHTER, KASSENARTEN, PROCESSED_DIR, ensure_dirs, parse_number, read_json
...
def parse_text_page(page: dict, metadata: dict) -> list[dict]:
    ...
    stichtag = infer_stichtag(text, int(metadata["jahr"]), int(metadata["monat"]))
    ...
    for raw_line in text.splitlines():
            ...
            if clean(line).startswith(clean(label)):
                ...
                if numbers:
                    for kassenart, number in values_to_kassenarten(numbers):
                        out.append(make_row(metadata, stichtag, kassenart, code, STRUCTURE_NAMES[code], "Zu", number, page_no))
        ...
        for kassenart, number in values_to_kassenarten(numbers):
            out.append(make_row(metadata, stichtag, kassenart, code, name, geschlecht, number, page_no))
        if geschlecht == "Zu":
            current_code = None
    return out
...
def normalize() -> list[dict]:
    ...
    for page in raw_pages:
        page_no = page["page"]
        stichtag = infer_stichtag(page.get("text", ""), int(metadata["jahr"]), int(metadata["monat"]))
            ...
            for table_row in table:
                ...
                if kassenarten and len(numeric_positions) >= len(kassenarten):
                    ...
                    for kassenart, (_, value) in zip(kassenarten, selected):
                        rows.append(make_row(metadata, stichtag, kassenart, code, name, "Zu", value, page_no))
                    ...
                    for kassenart in KASSENARTEN:
                        for geschlecht in GESCHLECHTER:
                            rows.append(make_row(metadata, stichtag, kassenart, code, name, geschlecht, selected[pos][1], page_no))
                            pos += 1
                else:
                    for _, value in numeric_positions[-1:]:
                        rows.append(make_row(metadata, stichtag, "Insgesamt", code, name, "Zu", value, page_no))
...
def make_row(metadata: dict, stichtag: str, kassenart: str, code: str, name: str, geschlecht: str, value: float, page_no: int) -> dict:
    return {
        "jahr": metadata["jahr"],
        "monat": metadata["monat"],
        "stichtag": stichtag,
        "kassenart": kassenart,
        "kennzahl_code": code,
        "kennzahl_name": name,
